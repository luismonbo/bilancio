...
from bilancio.storage.models import CategorizationRule, User
...
async def test_create_rule_writes_audit_log(db: AsyncSession) -> None:
    ...
    svc = RuleService(db)
    rule = await svc.create(user_id=user.id, pattern="X", pattern_type="contains", category="Y")

    logs = (await db.execute(select(AuditLog).where(AuditLog.actor_user_id == user.id))).scalars().all()
    assert any(log.action == "create" and log.entity_type == "categorization_rule" for log in logs)
...
async def test_list_returns_only_user_rules(db: AsyncSession) -> None:
    ...
    await svc.create(user_id=user_a.id, pattern="A", pattern_type="contains", category="Cat")
    await svc.create(user_id=user_b.id, pattern="B", pattern_type="contains", category="Cat")
...
async def test_list_returns_rules_sorted_by_priority_desc(db: AsyncSession) -> None:
    ...
    await svc.create(user_id=user.id, pattern="Low", pattern_type="contains", category="C", priority=1)
    await svc.create(user_id=user.id, pattern="High", pattern_type="contains", category="C", priority=50)
    await svc.create(user_id=user.id, pattern="Mid", pattern_type="contains", category="C", priority=10)
...
async def test_update_rule(db: AsyncSession) -> None:
    ...
    svc = RuleService(db)
    rule = await svc.create(user_id=user.id, pattern="Old", pattern_type="contains", category="OldCat")
...
async def test_update_rule_writes_audit_log(db: AsyncSession) -> None:
    ...
    logs = (await db.execute(select(AuditLog).where(AuditLog.actor_user_id == user.id))).scalars().all()
    assert any(log.action == "update" and log.entity_type == "categorization_rule" for log in logs)
...
async def test_update_rule_not_owned_raises(db: AsyncSession) -> None:
    ...
    svc = RuleService(db)
    rule = await svc.create(user_id=user_a.id, pattern="X", pattern_type="contains", category="Y")
...
async def test_delete_rule_writes_audit_log(db: AsyncSession) -> None:
    ...
    logs = (await db.execute(select(AuditLog).where(AuditLog.actor_user_id == user.id))).scalars().all()
    assert any(log.action == "delete" and log.entity_type == "categorization_rule" for log in logs)
...
async def test_yaml_export_contains_rules(db: AsyncSession) -> None:
    ...
    svc = RuleService(db)
    await svc.create(user_id=user.id, pattern="Esselunga", pattern_type="contains", category="Groceries", priority=10)
    await svc.create(user_id=user.id, pattern="ILIAD", pattern_type="exact", category="Utilities", priority=20)
...
async def test_yaml_round_trip(db: AsyncSession) -> None:
    ...
    await svc.create(user_id=user.id, pattern="Trenitalia", pattern_type="starts_with", category="Transport", priority=15)
