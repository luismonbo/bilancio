...
async def test_me_rejects_invalid_token(auth_client: AsyncClient) -> None:
    response = await auth_client.get("/me", headers={"Authorization": "Bearer bad-token"})
    assert response.status_code == 401


async def test_me_accepts_valid_token(auth_client: AsyncClient, db: AsyncSession) -> None:
...
async def test_me_rejects_revoked_token(auth_client: AsyncClient, db: AsyncSession) -> None:
...
async def test_me_rejects_disabled_user(auth_client: AsyncClient, db: AsyncSession) -> None:
    user = User(
